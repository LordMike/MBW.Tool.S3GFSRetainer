...
os.environ["S3_GFS_REGEX"] = r"Automatic_backup_\d+\.\d+\.\d+_(\d{4}-\d{2}-\d{2}_\d{2}\.\d{2})_"
os.environ["S3_GFS_TIMESTAMP_FORMAT"] = "%Y-%m-%d_%H.%M"
...
def test_retention_last_three_months_listing():
    ...
    keep_keys = [key for key, decision, _tag in decisions if decision == "keep"]
    assert len(keep_keys) == 6
    ...
    assert set(keep_keys) == expected_kept

    latest_key = "Automatic_backup_2026.1.0_2026-01-09_04.45_52003641.metadata.json"
    result_map = {key: (decision, tag) for key, decision, tag in decisions}
    assert result_map[latest_key][1] == "month,week,day"
